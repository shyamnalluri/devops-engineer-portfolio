...
export default function NotesIndexPage() {
  ...
  const notesByCategory = CATEGORIES.map((cat) => {
    if (cat.key === 'Other') {
      ...
      return { ...cat, notes: notes.filter((n) => !used.has(n.slug)) } as Category & { notes: ReturnType<typeof getAllNotes> };
    }
  ...
  const filteredNotesByCategory = notesByCategory.map((c) => ({
    ...
    notes: c.notes.filter((n) =>
      !q || n.title.toLowerCase().includes(q) || (n.summary || '').toLowerCase().includes(q) || lower(n.tags).some((t) => t.includes(q))
    ),
  }));

  // Sort: non-empty categories first; empty at bottom dimmed
  const sortedCategories = [...filteredNotesByCategory].sort((a, b) => (b.notes.length - a.notes.length));
  ...
  return (
    <main className="min-h-screen bg-black text-white px-6 py-24">
      <div className="section-wrap">
        ...
        <p className="section-subtitle mt-4">Browse by topic. Expand a section to see related notes.</p>

        {/* Tabs: Notes / Modules */}
        <div className="mt-5 mb-2 flex items-center justify-center gap-2">
          <a href="/notes" className="px-4 py-2 rounded-full text-sm font-medium bg-gradient-to-r from-orange-600 to-red-600 text-white shadow-red-600/20">Notes</a>
          <a href="/notes/modules" className="px-4 py-2 rounded-full text-sm font-medium bg-gray-800/60 text-gray-300 hover:bg-gray-700/60 hover:text-white border border-gray-700/50">Modules</a>
        </div>
        ...
        <form method="get" className="mt-6 mb-6">
          <div className="flex items-center gap-3">
            ...
            />
            {q && (
              <a href="/notes" className="text-xs text-gray-400 hover:text-gray-200">Clear</a>
            )}
          </div>
        </form>
        <div className="space-y-4">
          {sortedCategories.map((cat) => (
            <details key={cat.key} className={`group border border-gray-800 rounded-lg overflow-hidden ${cat.notes.length === 0 ? 'opacity-60' : ''}`} open={cat.notes.length > 0 ? false : false}>
              <summary className="list-none cursor-pointer select-none px-5 py-4 flex items-center justify-between bg-gray-900/40 hover:bg-gray-900/60 transition-colors">
                ...
                <span className="text-xs px-2 py-1 rounded-full bg-gray-800 text-gray-300 border border-gray-700 ml-auto">{cat.notes.length}</span>
              </summary>
              {cat.notes.length > 0 ? (
                      ...
                      <Link href={`/notes/${note.slug}`} className="block p-5 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:ring-offset-2 focus:ring-offset-black">
                        <div className="flex items-start justify-between gap-6">
                          <div className="flex-1">
                            ...
                            <time className="block text-[11px] text-gray-500 mb-2">{new Date(note.date).toLocaleDateString()}</time>
                            ...
                            {(shown.length > 0 || hiddenCount > 0) && (
                              <div className="mt-3 flex flex-wrap gap-2">
                                ...
                                {hiddenCount > 0 && (
                                  <span className="text-xs px-2 py-1 rounded-full bg-gray-800 text-gray-400 border border-gray-700">+{hiddenCount}</span>
                                )}
                              </div>
                            )}
                          </div>
