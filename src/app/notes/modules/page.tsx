...
export default function ModulesIndexPage() {
  ...
  const clouds: Array<'all' | 'aws' | 'azure' | 'gcp' | 'multi'> = [
    'all',
    'aws',
    'azure',
    'gcp',
    ...
    <main className="min-h-screen bg-black text-white px-6 py-24">
      <div className="section-wrap">
        ...
        <p className="section-subtitle mt-4">
          Copy usage blocks, explore inputs/outputs, and ship faster.
        </p>
        ...
        <form method="get" className="mt-6 mb-6 flex items-center gap-3">
          ...
          <select
            name="cloud"
            defaultValue={cloud}
            className="bg-gray-900/70 border border-gray-700/60 rounded-full px-3 py-2 text-sm text-slate-200"
          >
            {clouds.map((c) => (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((m) => (
            <Link
              key={m.slug}
              href={`/notes/modules/${m.slug}`}
              className="group mobile-card bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-gray-700/50 rounded-xl sm:rounded-2xl p-4 hover:border-red-500/30 transition-colors"
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {m.cloud?.toUpperCase()} {m.maturity === 'preview' ? '• Preview' : ''}
                  </div>
                </div>
                {m.version && (
                  <span className="text-[11px] px-2 py-1 rounded-full bg-gray-800 text-gray-300 border border-gray-700">
                    v{m.version}
                  </span>
                )}
              </div>
